...
from elanka_chat_ai.rag.retriever import DocumentRetriever
from elanka_chat_ai.rag.generator import AnswerGenerator
from elanka_chat_ai.storage.mongodb import get_storage
...
class RAGPipeline:
    ...
    def __init__(
        ...
        self.executor = ThreadPoolExecutor(max_workers=4)  # For async DB operations
    ...
    def _get_history_from_db(self, session_id: str) -> List[Dict]:
        """Fetch the last 6 messages from MongoDB for a session to use as context."""
        if not session_id:
            return []
        try:
            from elanka_chat_ai.storage.mongodb import get_storage
            with get_storage() as storage:
                session = storage.chat_sessions.find_one({"session_id": session_id})
                if session and "messages" in session:
                    # Retrieve the last 6 messages (3 turns)
                    return [{"role": m["role"], "content": m["content"]} for m in session["messages"][-6:]]
        except Exception as e:
            print(f"Warning: Failed to fetch chat history from DB: {e}")
        return []
    ...
    def ask(
        ...
        # Step 2: Retrieve conversation history from DB or memory
        conversation = []
        if use_history:
            if session_id:
                conversation = self._get_history_from_db(session_id)
            else:
                conversation = self.conversation_history[-6:]
        
        if stream:
            # Return generator for streaming (note: streaming doesn't support token tracking easily)
            ...
            # Step 3: Save to Memory (In-Memory fallback)
