...
from elanka_chat_ai.api.models import ChatRequest, ChatResponse, Source, ErrorResponse, ChatbotSettings, UpdateChatbotSettingsRequest
from elanka_chat_ai.api.dependencies import get_rag_pipeline, get_current_user, get_user_from_api_key, get_storage
...
@router.delete(
    "/session/{session_id}",
    summary="Delete conversation session history",
    description="Delete a specific chat session's conversation history."
)
async def delete_session_history(
    session_id: str,
    storage: MongoDBStorage = Depends(get_storage)
):
    """Delete conversation session history."""
    try:
        success = storage.delete_chat_session(session_id)
        return {"success": success, "message": "Session history deleted"}
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail={
...
async def update_chatbot_settings(
    ...
    try:
        ...
        print(f"Error updating chatbot settings: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=str(e))
...
@router.get(
    ...
    description="Returns client-specific aggregated statistics on token usage."
)
async def get_token_usage(
    ...
    """
    Dashboard endpoint: Get token usage analytics.
    Authentication: JWT token
    """
    try:
