...
from elanka_chat_ai.api.dependencies import get_current_active_superuser, get_storage
from elanka_chat_ai.storage.mongodb import MongoDBStorage
from pydantic import BaseModel
import secrets
from elanka_chat_ai.api.security import get_password_hash
...
async def create_client_manually(
    ...
    email_sent = email_service.send_welcome_email(
        ...
        client_name=user_in.name,
        temporary_password=temp_password
    )
    
    # Return the password so Admin can copy it
    return {
        "message": "User created" + (" and email sent" if email_sent else ", but failed to send email"),
        "user_id": user_id,
        "email": user_in.email,
        "temporary_password": temp_password,
...
async def get_system_health(
    ...
    from elanka_chat_ai.api.health_monitor import get_system_metrics, get_database_metrics, get_qdrant_metrics
