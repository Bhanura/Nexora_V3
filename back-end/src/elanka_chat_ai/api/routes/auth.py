...
from elanka_chat_ai.api.security import create_access_token, verify_password, get_password_hash, ACCESS_TOKEN_EXPIRE_MINUTES
from elanka_chat_ai.api.dependencies import get_storage, get_current_user
from elanka_chat_ai.storage.mongodb import MongoDBStorage
from elanka_chat_ai.services.email_service import get_email_service
...
async def create_new_api_key(
    ...
    # Save allowed domains if passed
    if request.allowed_domains:
        domains = [d.strip().lower() for d in request.allowed_domains if d.strip()]
...
async def update_api_key(
    ...
    """Update fields of an API key (name and/or allowed_domains)."""
    updates = {}
    ...
    success = storage.update_api_key(key_id, current_user["_id"], updates)
    if not success:
...
async def forgot_password(
    ...
    """Initiate password recovery by generating a 6-digit code."""
    user = storage.users.find_one({"email": request.email})
    if user:
        import random
        ...
        # Print to console for local testing/verification as fallback
        print(f"\n==========================================")
        print(f"[RECOVERY CODE] Code for {request.email}: {code}")
        print(f"==========================================\n")
        
    return {"message": "If the email is registered, a password recovery code has been sent."}
...
async def reset_password(
    ...
    user = storage.users.find_one({"email": request.email})
    if not user:
        raise HTTPException(status_code=400, detail="Invalid email, code, or code expired")
        
    reset_code = user.get("reset_code")
    reset_code_expires = user.get("reset_code_expires")
    
    if not reset_code or reset_code != request.code:
        raise HTTPException(status_code=400, detail="Invalid email, code, or code expired")
        
    if not reset_code_expires or reset_code_expires < datetime.utcnow():
        raise HTTPException(status_code=400, detail="Invalid email, code, or code expired")
