...
class TextIngestRequest(BaseModel):
    ...
    title: str = Field(..., description="Title or source label for the text", min_length=1, max_length=200)
    text: str = Field(..., description="Raw text content to ingest", min_length=10)
    
    class Config:
        json_schema_extra = {
            "example": {
                "title": "Return Policy Guide",
                "text": "All items can be returned within 30 days of purchase for a full refund. The items must be unused and in original packaging..."
            }
        }
...
class ChatbotSettings(BaseModel):
    ...
    theme_color: str = Field(default="#EF4444", description="Custom theme color (Hex code)")
